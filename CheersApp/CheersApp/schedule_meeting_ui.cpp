...
CSDKSheduleMeetingUIGroup::CSDKSheduleMeetingUIGroup()
{
	...
	//m_dial_in_country = NULL; //EDIT: removed dial_in_country from files
...
void CSDKSheduleMeetingUIGroup::InitWindow( CPaintManagerUI& ui_mgr, CSDKSheduleMeetingUIMgr* main_frame_ )
{
	...
	//m_edit_dial_in_btn = static_cast<CButtonUI* >(ui_mgr.FindControl(L"edit_dial_in_country_btn")); //EDIT: removed dial_in_country from files
	...
	//InitDialInCountry(); //EDIT: removed dial_in_country from files
}
	...
	if (result == ZOOM_SDK_NAMESPACE::PREMETAPIRET_SUCCESS)
	{
		...
		if (pMeetingItemInfo)
		{
			meeting_info = std::wstring(pMeetingItemInfo->GetMeetingTopic())+L"\n"
				+std::wstring( pMeetingItemInfo->GetInviteEmailSubject());

			if (pMeetingItemInfo->GetInviteEmailContent())
				meeting_info += L"\n" + std::wstring(pMeetingItemInfo->GetInviteEmailContent());
			else
				pMeetingItemInfo->AsyncGetInviteEmailContent();
...
void CSDKSheduleMeetingUIGroup::onGetInviteEmailContent(ZOOM_SDK_NAMESPACE::PremeetingAPIResult result, UINT64 meetingUniqueID, const wchar_t* content)
{
	if (result == ZOOM_SDK_NAMESPACE::PREMETAPIRET_SUCCESS)
	{
		if (m_schedule_meeting_info && content)
		{
			m_schedule_meeting_info->Clear();
			std::wstring meeting_info;
			meeting_info = m_schedule_meeting_info->GetText();
			meeting_info += L"\n" + std::wstring(content);
			m_schedule_meeting_info->SetText(meeting_info.c_str());
		}
	}
