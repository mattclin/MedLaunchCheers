...
CSDKGeneralSettingsUIGroup::CSDKGeneralSettingsUIGroup()
{
	...
	m_cAutoJoinAudio = NULL;
	m_cAutoJoinAudio_false = NULL;
...
void CSDKGeneralSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	...
	m_cAutoJoinAudio = static_cast<CButtonUI*>(ui_mgr.FindControl(_T("c_auto_join_audio")));
	m_cAutoJoinAudio_false = static_cast<CButtonUI*>(ui_mgr.FindControl(_T("c_auto_join_audio_false")));
...
void CSDKGeneralSettingsUIGroup::GetGeneralSettingsFlags()
{
	...
	bChecked = m_generalSettingsWorkFlow.IsAutoJoinAudioEnabled_settings();
	if(m_cAutoJoinAudio)
	{
		m_cAutoJoinAudio->SetEnabled(bChecked);
	}
...
void CSDKGeneralSettingsUIGroup::Notify(TNotifyUI & msg)
{
	if(msg.sType == _T("click"))
	{
		...
		else if (msg.pSender == m_cAutoJoinAudio_false) {
			DoAutoJoinAudioChkClick_settings(false);
		}
		else if (msg.pSender == m_cAutoJoinAudio) {
			DoAutoJoinAudioChkClick_settings(true);
		}
	}
}

void CSDKGeneralSettingsUIGroup::DoAutoJoinAudioChkClick_settings(bool b)
{
	if (NULL == m_cAutoJoinAudio || NULL == m_cAutoJoinAudio_false)
		return;
	bool bChecked = !m_cAutoJoinAudio->IsEnabled();
	unsigned int hover = 0xff9bd0fa;
	m_cAutoJoinAudio_false->SetHotBkColor(hover);
	m_cAutoJoinAudio->SetHotBkColor(hover);
	if (b) {
		m_cAutoJoinAudio_false->SetEnabled(true);
		m_cAutoJoinAudio->SetEnabled(false);
		bChecked = false;
		DWORD c = 0xffffffff;
		m_cAutoJoinAudio_false->SetBkColor(c);
		unsigned int c2 = 0xff189ff5;
		m_cAutoJoinAudio->SetBkColor(c2);
	} //if yes clicked, enable no, disable yes
	if (!b) {
		m_cAutoJoinAudio->SetEnabled(true);
		m_cAutoJoinAudio_false->SetEnabled(false);
		bChecked = true;
		DWORD c = 0xffffffff;
		m_cAutoJoinAudio->SetBkColor(c);
		unsigned int c2 = 0xff189ff5;
		m_cAutoJoinAudio_false->SetBkColor(c2);
	} //if no clicked, enable yes, disable no
	m_audioSettingsWorkFlow_settings.EnableAutoJoinAudio(bChecked);
}
...
void CSDKAudioSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	...
	//m_chkAutoJoinAudio = static_cast<CButtonUI*>(ui_mgr.FindControl(_T("chk_auto_join_audio")));
	//m_chkAutoJoinAudio_false = static_cast<CButtonUI*>(ui_mgr.FindControl(_T("chk_auto_join_audio_false")));
...
void CSDKAudioSettingsUIGroup::GetAudioSettingsFlags()
{
	...
	{
		m_chkAutoJoinAudio->SetEnabled(bChecked);
	}*/
...
void CSDKAudioSettingsUIGroup::Notify(TNotifyUI& msg)
{
	if(msg.sType == _T("click"))
	{
		...
		{
			DoAutoJoinAudioChkClick(1);
		}
		/*else if (msg.pSender == m_chkAutoJoinAudio_false) {
			DoAutoJoinAudioChkClick(0);
		}*/
...
/*void CSDKAudioSettingsUIGroup::DoAutoJoinAudioChkClick(int num)
{
	if(NULL == m_chkAutoJoinAudio)
		return;
	bool bChecked = !m_chkAutoJoinAudio->IsEnabled();
	if (m_chkAutoJoinAudio->IsEnabled() && num == 1) {
		m_chkAutoJoinAudio_false->SetEnabled(true);
		m_chkAutoJoinAudio->SetEnabled(false);
		bChecked = false;
	} //if yes clicked, enable no, disable yes
	
   if (m_chkAutoJoinAudio_false->IsEnabled() && num == 0) {
		m_chkAutoJoinAudio->SetEnabled(true);
		m_chkAutoJoinAudio_false->SetEnabled(false);
		bChecked = true;
	} //if no clicked, enable yes, disable no
...
CSDKShareSettingsUIGroup::CSDKShareSettingsUIGroup()
{
	...
	m_chkUseHardwareAccelerationAnnotaion = NULL;
...
void CSDKShareSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	...
	m_chkUseHardwareAccelerationAnnotaion = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_use_hardware_acceleration_annotation")));
...
void CSDKShareSettingsUIGroup::GetShareSettingsFlags()
{
	...
	{
		m_chkRemoteControl->SetCheck(false);
	}
	...
	bChecked = m_shareSettingsWorkFlow.IsAnnotateHardwareAcceleration();
	if (m_chkUseHardwareAccelerationAnnotaion)
	{
		m_chkUseHardwareAccelerationAnnotaion->SetCheck(bChecked);
	}
	bool bIsSupport = m_shareSettingsWorkFlow.IsCurrentOSSupportAccelerateGPUWhenShare();
...
void CSDKShareSettingsUIGroup::Notify(TNotifyUI& msg)
{
	if (msg.sType == _T("click"))
	{
		...
		else if (msg.pSender == m_chkUseHardwareAccelerationAnnotaion)
		{
			DoUseHardwareAccelerationAnnotationChkClick();
		}
...
void CSDKShareSettingsUIGroup::DoUseHardwareAccelerationAnnotationChkClick()
{
	if (NULL == m_chkUseHardwareAccelerationAnnotaion)
		return;
	bool bChecked = !m_chkUseHardwareAccelerationAnnotaion->GetCheck();
...
CSDKUICustomSettingsUIGroup::CSDKUICustomSettingsUIGroup()
{
	...
	//m_chkDialInSubTab = NULL; //EDIT: removed dial_in_country from files
...
void CSDKUICustomSettingsUIGroup::InitWindow(CPaintManagerUI& ui_mgr, CSDKSettingsUIMgr* main_frame_)
{
	...
	//m_chkDialInSubTab = static_cast<CCheckBoxUI*>(ui_mgr.FindControl(_T("chk_dial_in_sub-tab"))); //EDIT: removed dial_in_country from files
...
void CSDKUICustomSettingsUIGroup::SetUICustomSettingsFlags()
{
	...
	//} ////EDIT: removed dial_in_country from files
